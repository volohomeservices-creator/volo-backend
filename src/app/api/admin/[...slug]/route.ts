  ...
  try {
    ...
    if (slug[0] === 'dashboard' && slug[1] === 'metrics') {
      ...
      const [
        ...
        supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).eq('role', 'customer').eq('is_active', true),
        supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).eq('role', 'customer').gte('created_at', weekStart),
        supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).eq('role', 'customer').eq('is_active', false),
        supabaseAdmin.from('reviews').select('*', { count: 'exact', head: true }).lte('rating', 3),
    ...
    if (slug[0] === 'workers' && slug.length === 1) {
      ...
      // Auto-reconcile orphaned ON_JOB status if no active booking exists
      const onJobWorkerIds = (workers || [])
        .filter((w: any) => w.status === 'ON_JOB')
        .map((w: any) => w.id);
      ...
      if (onJobWorkerIds.length > 0) {
        const { data: activeBookings } = await supabaseAdmin
          .from('bookings')
          .select('worker_id')
          .in('worker_id', onJobWorkerIds)
          .in('status', ['WORKER_ACCEPTED', 'ON_THE_WAY', 'ARRIVED', 'IN_PROGRESS']);

        const activeWorkerIdSet = new Set((activeBookings || []).map((b: any) => b.worker_id));

        for (const w of (workers || [])) {
          if (w.status === 'ON_JOB' && !activeWorkerIdSet.has(w.id)) {
            w.status = 'ONLINE';
            // Self-heal DB row in background
            supabaseAdmin
              .from('workers')
      ...
      return NextResponse.json({
        workers: (workers || []).map((w: any) => ({
          ...
          commission_wallet_balance: Number((Array.isArray(w.worker_wallets) ? w.worker_wallets[0]?.balance : w.worker_wallets?.balance) || 0),
    ...
    if (slug[0] === 'workers' && slug.length === 2) {
      ...
      if (workerData.service_category_ids && workerData.service_category_ids.length > 0) {
        const { data: categories } = await supabaseAdmin
          ...
          .select('name')
          .in('id', workerData.service_category_ids);
        if (categories) {
      ...
      return NextResponse.json({
        id: workerData.id,
        full_name: workerData.users?.full_name,
        phone: workerData.users?.phone,
        email: workerData.users?.email,
        ...
        commission_wallet_balance: (Array.isArray((workerData as any).worker_wallets) ? (workerData as any).worker_wallets[0]?.balance : (workerData as any).worker_wallets?.balance) || 0,
        rating: workerData.rating,
        total_jobs: workerData.total_jobs,
        created_at: workerData.created_at,
        is_active: workerData.users?.is_active,
        wallet_transactions: walletTxns || [],
        settlements: settlements || [],
        dob: workerData.dob,
        worker_id_code: workerData.worker_id_code,
        skills: workerData.worker_profiles?.skills || [],
    ...
    if (slug[0] === 'customers' && slug.length === 1) {
      ...
      let query = supabaseAdmin
        .from('users')
        .select('id, full_name, phone, email, is_active, avatar_url, created_at, bookings(id, total_amount, created_at, status)')
        .eq('role', 'customer');
      ...
      return NextResponse.json({
        customers: (customers || []).map((c: any) => {
          ...
          const totalSpend = bookingsList.reduce((sum: number, b: any) => sum + (Number(b.total_amount) || 0), 0);
          const latestBooking = bookingsList.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0];
          ...
          return {
            ...
            last_activity: latestBooking?.created_at || c.created_at
          };
        }),
        total: count || 0,
        page,
    ...
    if (slug[0] === 'customers' && slug.length === 2) {
      ...
      // Fetch last 20 bookings and reviews
      ...
      const avgRating = reviewsList.length > 0
        ? Number((reviewsList.reduce((sum: number, r: any) => sum + (Number(r.rating) || 0), 0) / reviewsList.length).toFixed(1))
        : null;
      ...
      return NextResponse.json({
        ...
        total_bookings: bookingsList.length,
        bookings: bookingsList.map((b: any) => ({
          id: b.id,
          status: b.status,
          total_amount: Number(b.total_amount) || 0,
          created_at: b.created_at,
          service_name: b.service_items?.name || 'Service'
        })),
        reviews: reviewsList
      });
    }
    ...
    if (slug[0] === 'bookings' && slug.length === 1) {
      ...
      // Parallel query for metrics counts
      const [
        ...
        supabaseAdmin.from('bookings').select('*', { count: 'exact', head: true }).eq('status', 'PENDING_ASSIGNMENT'),
        supabaseAdmin.from('bookings').select('*', { count: 'exact', head: true }).in('status', ['ASSIGNED', 'ACCEPTED']),
        supabaseAdmin.from('bookings').select('*', { count: 'exact', head: true }).in('status', ['IN_PROGRESS', 'STARTED']),
        supabaseAdmin.from('bookings').select('*', { count: 'exact', head: true }).eq('status', 'COMPLETED')
      ]);
      ...
      return NextResponse.json({
        bookings: (bookings || []).map((b: any) => ({
          ...
          notes: b.notes,
          otp: b.otp,
          service_id: b.service_items?.id,
          service_name: b.service_items?.name || 'General Service',
          service_category: b.service_items?.service_categories?.name || 'Home Services',
          ...
          worker_id: b.worker?.id || null,
          worker_name: b.worker?.users?.full_name || null,
          worker_phone: b.worker?.users?.phone || null,
          worker_avatar: b.worker?.users?.avatar_url || null
        })),
        metrics: {
    ...
    if (slug[0] === 'bookings' && slug.length === 2) {
      ...
      return NextResponse.json({
        ...
        worker: bookingData.worker ? {
          ...
          full_name: bookingData.worker.users?.full_name,
          phone: bookingData.worker.users?.phone,
        } : null,
        images: imageUrls,
    ...
    if (slug[0] === 'services' && slug.length === 1) {
      const [{ data: categories, error }, { data: bookingsAgg }] = await Promise.all([
        ...
        supabaseAdmin
          .from('bookings')
          .select('service_item_id, total_amount, status, reviews(rating)')
      ]);

      if (error) throw error;

      // Group bookings & reviews by service_item_id
      const bookingStatsMap: Record<string, { total_bookings: number; total_revenue: number; ratings: number[] }> = {};
      ...
      const enrichedCategories = (categories || []).map((cat: any) => ({
        ...cat,
        service_items: (cat.service_items || []).map((item: any) => {
          const stats = bookingStatsMap[item.id] || { total_bookings: 0, total_revenue: 0, ratings: [] };
          const avgRate = stats.ratings.length > 0
            ? Number((stats.ratings.reduce((a, c) => a + c, 0) / stats.ratings.length).toFixed(1))
            : 4.8;
    ...
    if (slug[0] === 'manual-assignment' && slug.length === 1) {
      ...
      if (bookingId) {
        ...
        if (targetJob) {
          const supabaseAdminAny: any = supabaseAdmin;
    ...
    if (slug[0] === 'settlements' && slug.length === 1) {
      ...
      const [
        ...
        supabaseAdmin
          .from('settlement_ledger')
          .select('*, worker:workers(id, users(full_name, phone, avatar_url))')
          .order('created_at', { ascending: false })
          .limit(50),
        supabaseAdmin
          .from('bookings')
          .select('id, total_amount, status, created_at, address_line, service_items(name, category_id, service_categories(name)), worker:workers(users(full_name, phone)), customer:users!bookings_customer_id_fkey(full_name)')
          .eq('status', 'COMPLETED')
          .order('created_at', { ascending: false })
          .limit(50),
        supabaseAdmin
          .from('workers')
          .select('id, commission_wallet_balance, status, kyc_status, users(full_name, phone, email, avatar_url)'),
        supabaseAdmin
          .from('worker_bank_accounts')
          .select('*, worker:workers(id, users(full_name, phone, email, avatar_url))')
          .order('created_at', { ascending: false }),
        supabaseAdmin
          .from('workers')
      ...
      // Format Ledger entries from settlement_ledger or real completed bookings
      const formattedLedger = (ledgerData && ledgerData.length > 0)
        ? ledgerData.map((l: any) => ({
            id: l.id,
            booking_id: l.booking_id || 'BK-DIRECT',
            worker_name: l.worker?.users?.full_name || 'Technician',
            worker_phone: l.worker?.users?.phone || '',
        ...
        ? completedBookings.map((b: any) => {
            ...
            return {
              ...
              service_name: b.service_items?.name || 'Service',
              customer_name: b.customer?.full_name || 'Customer',
              worker_name: b.worker?.users?.full_name || 'Technician',
      ...
      // Format Bank Accounts from real worker_bank_accounts table only
      const formattedBanks = (bankAccountsData && bankAccountsData.length > 0)
        ? bankAccountsData.map((b: any) => ({
            ...
            account_holder_name: b.account_holder_name || b.worker?.users?.full_name || 'Account Holder',
      ...
      // Calculate total platform commission & total worker payouts from real data
      ...
      // Also incorporate completed bookings if ledger batches are still pending
      ...
      // Group top earning services by real commission
      const serviceCommissionMap: Record<string, { name: string; commission: number }> = {};
      (completedBookings || []).forEach((b: any) => {
        const sName = b.service_items?.name || 'General Service';
      ...
      const topEarningServices = Object.values(serviceCommissionMap)
      ...
      return NextResponse.json({
        metrics: {
          ...
          ready_technicians: readyWorkersCount || (workersData || []).filter((w: any) => w.kyc_status === 'APPROVED').length,
    ...
    if (slug[0] === 'wallets' && slug.length === 1) {
      ...
      const [workersRes, txnsRes] = await Promise.all([
        supabaseAdmin
          .from('workers')
          .select('id, status, kyc_status, commission_wallet_balance, users!inner(id, full_name, phone, avatar_url), worker_wallets(balance, minimum_balance, is_active)')
          .order('created_at', { ascending: false }),
        supabaseAdmin
          .from('wallet_transactions')
          .select('*')
      ...
      // Map latest transaction per worker
      const latestTxnMap: Record<string, any> = {};
      txns.forEach((t: any) => {
      ...
      const formattedWallets = workers.map((w: any) => {
        const balance = Number(w.commission_wallet_balance || (Array.isArray(w.worker_wallets) ? w.worker_wallets[0]?.balance : w.worker_wallets?.balance) || 0);
        const thresholdLimit = Number((Array.isArray(w.worker_wallets) ? w.worker_wallets[0]?.minimum_balance : w.worker_wallets?.minimum_balance) || 5000);
        
        totalWalletReserve += balance;
        if (w.status !== 'SUSPENDED') activeWalletsCount++;
        if (balance < thresholdLimit) lowBalanceCount++;

        const thresholdStatus = balance >= thresholdLimit
          ? 'HEALTHY'
          : (balance > 0 ? 'LOW_BALANCE' : 'CRITICAL');
        ...
        return {
          ...
          full_name: w.users?.full_name || 'Technician',
          phone: w.users?.phone || 'N/A',
          avatar_url: w.users?.avatar_url || null,
          balance,
      ...
      if (thresholdFilter && thresholdFilter !== 'ALL') {
        filtered = filtered.filter((w: any) => w.threshold_status === thresholdFilter);
      }

      if (eligibilityFilter && eligibilityFilter !== 'ALL') {
        filtered = filtered.filter((w: any) => w.job_eligibility === eligibilityFilter);
      }
    ...
    // O. payments (Platform Transactions)
    if (slug[0] === 'payments' && slug.length === 1) {
      ...
      const formatted = (rawPayments || []).map((p: any) => {
        const amt = Number(p.amount || p.booking?.total_amount || 0);
        ...
        // Map status to clean display status
        ...
        return {
          ...
          customer_name: p.booking?.customer?.full_name || 'Customer',
          customer_phone: p.booking?.customer?.phone || 'N/A',
          booking_id: p.booking_id || '',
          booking_reference: `B-${p.booking_id ? p.booking_id.replace(/\D/g, '').slice(-5) : '17384'}`,
          service_name: p.booking?.service_items?.name || 'Home Service',
          amount: amt,
      ...
      if (statusFilter && statusFilter !== 'ALL') {
        filtered = filtered.filter((p: any) => p.status.toUpperCase() === statusFilter.toUpperCase());
      }

      if (methodFilter && methodFilter !== 'ALL') {
        filtered = filtered.filter((p: any) => p.method.toUpperCase() === methodFilter.toUpperCase());
      }
...
export async function POST(
  ...
  try {
    ...
    if (slug[0] === 'wallets' && slug[1] === 'adjust') {
      ...
      // Fetch current worker balance
      const { data: workerData } = await supabaseAdmin
        .from('workers')
        .select('commission_wallet_balance')
        .eq('id', worker_id)
        .single();

      const currentBalance = Number(workerData?.commission_wallet_balance || 0);
      const newBalance = type === 'CREDIT' ? currentBalance + numAmount : Math.max(0, currentBalance - numAmount);

      await supabaseAdmin
        .from('workers')
        .update({ commission_wallet_balance: newBalance })
        .eq('id', worker_id);
    ...
    if (slug[0] === 'services' && slug[1] === 'categories' && slug.length === 2) {
      const { data: { name, icon_url, sort_order }, errorResponse } = await validateBody(request, z.any());
    ...
    // A.3 services/categories/[id]
    if (slug[0] === 'services' && slug[1] === 'categories' && slug.length === 3) {
      const categoryId = slug[2];
      const { data: updateFields, errorResponse } = await validateBody(request, z.any());
    if (errorResponse) return errorResponse;
    ...
    if (slug[0] === 'services' && slug.length === 1) {
      const { data: { category_id, name, description, base_price, estimated_mins, icon_url }, errorResponse } = await validateBody(request, z.any());
    if (errorResponse) return errorResponse;
    ...
    // B. manual-assignment
    if (slug[0] === 'manual-assignment' && slug.length === 1) {
      const { data: { booking_id, worker_id }, errorResponse } = await validateBody(request, z.any());
    if (errorResponse) return errorResponse;
    ...
    if (slug[0] === 'manual-assignment' && slug[1] === 'reassign') {
      const { data: { booking_id, new_worker_id }, errorResponse } = await validateBody(request, z.any());
    if (errorResponse) return errorResponse;
    ...
    if (slug[0] === 'workers' && slug.length === 3 && slug[2] === 'categories') {
      const workerId = slug[1];
      const { data: { categoryIds }, errorResponse } = await validateBody(request, z.any());
    if (errorResponse) return errorResponse;
    ...
    // F. settings
    if (slug[0] === 'settings' && slug.length === 1) {
      const { data: settingsPayload, errorResponse } = await validateBody(request, z.any());
    if (errorResponse) return errorResponse;
