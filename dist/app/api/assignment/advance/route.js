...
// src/app/api/assignment/advance/route.ts
var route_exports = {};
...
async function POST(request) {
  try {
    ...
    const { data: expiredQueues, error: queryErr } = await supabaseAdmin.from("assignment_queue").select("id, booking_id, status, group_expires_at").or(`and(status.eq.BROADCASTING,group_expires_at.lt.${nowIso}),and(status.eq.QUEUED,group_expires_at.lt.${nowIso})`).limit(50);
    if (queryErr) {
