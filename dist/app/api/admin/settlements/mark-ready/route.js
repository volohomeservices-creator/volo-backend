...
// src/app/api/admin/settlements/mark-ready/route.ts
var route_exports = {};
...
// packages/shared-lib/src/lib/settlement-engine.ts
async function markReadyForPayout(batchId, adminId) {
  try {
    const { data: batch, error: fetchErr } = await supabaseAdmin.from("settlement_batches").select("status").eq("id", batchId).single();
    ...
    const { error: batchUpdateErr } = await supabaseAdmin.from("settlement_batches").update({ status: "READY_FOR_PAYOUT", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", batchId);
    if (batchUpdateErr) throw batchUpdateErr;
    const { error: ledgerUpdateErr } = await supabaseAdmin.from("settlement_ledger").update({ status: "READY_FOR_PAYOUT", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("settlement_batch_id", batchId);
    if (ledgerUpdateErr) throw ledgerUpdateErr;
    await logAuditAction({
      admin_id: adminId,
      action: "SETTLEMENT_READY_FOR_PAYOUT" /* SETTLEMENT_READY_FOR_PAYOUT */,
