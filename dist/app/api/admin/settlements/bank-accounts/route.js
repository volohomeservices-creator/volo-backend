...
// src/app/api/admin/settlements/bank-accounts/route.ts
var route_exports = {};
...
async function GET(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin2.from("worker_bank_accounts").select("*, workers(users(full_name, phone))").order("created_at", { ascending: false });
...
async function POST(request) {
  try {
    ...
    const { data: account, error: updateErr } = await supabaseAdmin2.from("worker_bank_accounts").update({ is_verified: isVerified, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", accountId).select("worker_id").single();
    if (updateErr || !account) throw updateErr;
    await logAuditAction({
      admin_id: session.user_id,
      action: "BANK_ACCOUNT_VERIFIED" /* BANK_ACCOUNT_VERIFIED */,
