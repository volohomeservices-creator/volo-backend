...
async function PUT(request, props) {
  try {
    ...
    const { title, subtitle, discount_label, action_url, background_color, image_name, image_url, placement, active } = body;
    const { data: updatedBanner, error } = await supabaseAdmin2.from("mobile_banners").update({
      title,
      subtitle,
      discount_label,
      action_url,
