...
async function GET(request) {
  try {
    ...
    const { data: banners, error } = await supabaseAdmin2.from("mobile_banners").select("*").order("created_at", { ascending: false });
...
async function POST(request) {
  try {
    ...
    const { data: newBanner, error } = await supabaseAdmin2.from("mobile_banners").insert(insertPayload).select("*").single();
    if (error) throw error;
...
async function PATCH(request) {
  try {
    ...
    if (discount_label !== void 0) updates.discount_label = discount_label;
    if (target_panel === "WORKER" || action_url && action_url.startsWith("/worker")) {
      updates.placement = "WORKER";
    } else if (placement !== void 0) {
      updates.placement = placement;
    }
    if (title !== void 0) updates.title = title;
    if (subtitle !== void 0) updates.subtitle = subtitle;
    if (action_url !== void 0) updates.action_url = action_url;
    if (button_text !== void 0 || cta_text !== void 0) updates.button_text = button_text || cta_text;
    if (image_url !== void 0) {
      updates.image_url = image_url;
      updates.image_name = image_url || image_name;
    }
    if (background_color !== void 0) updates.background_color = background_color;
    const { data: updatedBanner, error } = await supabaseAdmin2.from("mobile_banners").update(updates).eq("id", id).select("*").single();
    if (error) throw error;
...
async function DELETE(request) {
  try {
    ...
    const { error } = await supabaseAdmin2.from("mobile_banners").delete().eq("id", id);
    if (error) throw error;
