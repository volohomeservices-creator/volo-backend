...
// src/app/api/admin/customers/[id]/refund/route.ts
var route_exports = {};
...
async function POST(request, { params }) {
  try {
    ...
    const { data: { amount, reason, bookingId }, errorResponse } = await validateBody(request, import_zod.z.any());
    ...
    const { data: wallet, error: walletError } = await supabaseAdmin2.from("customer_wallets").select("balance").eq("customer_id", customerId).single();
    if (walletError || !wallet) {
      return NextResponse.json({ error: "Customer wallet not found." }, { status: 404 });
    }
    const { error: txError } = await supabaseAdmin2.from("customer_wallet_transactions").insert({
    ...
    const { error: updateError } = await supabaseAdmin2.from("customer_wallets").update({ balance: newBalance, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("customer_id", customerId);
    if (updateError) throw updateError;
    await supabaseAdmin2.from("admin_audit_logs").insert({
