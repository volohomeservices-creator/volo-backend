...
async function GET(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin2.from("promo_codes").select("*, created_by_user:users(full_name)").order("created_at", { ascending: false });
...
async function POST(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin2.from("promo_codes").insert({
      ...
      valid_from: valid_from || (/* @__PURE__ */ new Date()).toISOString(),
...
async function PATCH(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin2.from("promo_codes").update({ active: Boolean(active) }).eq("id", id).select().single();
    if (error) throw error;
    return NextResponse.json({ promo_code: data });
  } catch (error) {
    return NextResponse.json({ error: error.message }, { status: error.status || 500 });
...
async function DELETE(request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "id is required" }, { status: 400 });
    const { error } = await supabaseAdmin2.from("promo_codes").delete().eq("id", id);
    if (error) throw error;
