...
async function POST(request, { params }) {
  try {
    ...
    const { data: booking, error: fetchErr } = await supabaseAdmin2.from("bookings").select("id, status, customer_id, total_amount, worker_id").eq("id", bookingId).single();
    if (fetchErr || !booking) {
    ...
    const { error: updateErr } = await supabaseAdmin2.from("bookings").update({
      status: "CANCELLED",
    ...
    if (refund_to_wallet && booking.customer_id && booking.total_amount) {
      ...
      const { data: existingWallet } = await supabaseAdmin2.from("customer_wallets").select("id, balance").eq("customer_id", booking.customer_id).maybeSingle();
      if (existingWallet) {
        await supabaseAdmin2.from("customer_wallets").update({ balance: Number(existingWallet.balance) + refundAmount }).eq("id", existingWallet.id);
      } else {
        await supabaseAdmin2.from("customer_wallets").insert({ customer_id: booking.customer_id, balance: refundAmount });
      }
      try {
        await supabaseAdmin2.from("customer_wallet_transactions").insert({
    ...
    if (booking.customer_id) {
      await dispatchNotification({
        ...
        title: "\u{1F4CB} Booking Cancelled",
        body: refund_to_wallet ? `Your booking has been cancelled. \u20B9${booking.total_amount} has been refunded to your wallet.` : `Your booking has been cancelled by admin. ${reason ? "Reason: " + reason : ""}`,
