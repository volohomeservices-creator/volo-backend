...
async function GET(request) {
  try {
    ...
    let query = supabaseAdmin2.from("referrals").select(`
        id,
        status,
        reward_amount,
        role,
...
async function PATCH(request) {
  try {
    ...
    const { data: { referral_id }, errorResponse } = await validateBody(request, import_zod2.z.any());
    if (errorResponse) return errorResponse;
    if (!referral_id) {
      return NextResponse.json({ error: "referral_id is required" }, { status: 400 });
    }
    const { data: referral, error: fetchErr } = await supabaseAdmin2.from("referrals").select("id, referrer_id, reward_amount, status, role").eq("id", referral_id).single();
    ...
    const { error: updateErr } = await supabaseAdmin2.from("referrals").update({ status: "REWARDED", rewarded_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", referral_id);
    if (updateErr) throw updateErr;
    const rewardAmount = Number(referral.reward_amount || 0);
    if (rewardAmount > 0 && referral.role === "customer") {
      try {
        const { data: wallet } = await supabaseAdmin2.from("customer_wallets").select("id, balance").eq("customer_id", referral.referrer_id).single();
        if (wallet) {
          await supabaseAdmin2.from("customer_wallets").update({ balance: Number(wallet.balance) + rewardAmount }).eq("id", wallet.id);
        } else {
          await supabaseAdmin2.from("customer_wallets").insert({ customer_id: referral.referrer_id, balance: rewardAmount });
        }
      } catch (_) {
      }
    }
    await dispatchNotification({
      ...
      title: "\u{1F389} Referral Reward Credited!",
