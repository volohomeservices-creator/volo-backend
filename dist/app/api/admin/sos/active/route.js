...
// src/app/api/admin/sos/active/route.ts
var route_exports = {};
...
async function GET(request) {
  try {
    ...
    const { data: alerts, error } = await supabaseAdmin2.from("sos_alerts").select("id, user_id, booking_id, lat, lng, status, created_at").eq("status", "ACTIVE").order("created_at", { ascending: false });
    if (error) {
    ...
    const userMap = {};
    if (userIds.length > 0) {
      const { data: users } = await supabaseAdmin2.from("users").select("id, full_name, phone, role").in("id", userIds);
      (users || []).forEach((u) => {
        userMap[u.id] = u;
      });
    }
    const populated = alerts.map((a) => __spreadProps(__spreadValues({}, a), {
...
async function PATCH(request) {
  try {
    ...
    const { data: { id }, errorResponse } = await validateBody(request, import_zod.z.any());
    if (errorResponse) return errorResponse;
    if (!id) {
      return NextResponse.json({ error: "SOS ID is required" }, { status: 400 });
    }
    const { error } = await supabaseAdmin2.from("sos_alerts").update({
      status: "RESOLVED",
