...
// src/app/api/admin/push-notify/route.ts
var route_exports = {};
...
async function POST(request) {
  try {
    ...
    if (target === "specific" && target_user_id) {
      ...
      const { data: users } = await supabaseAdmin2.from("users").select("id").eq("role", "customer").eq("is_active", true);
      ...
      const { data: users } = await supabaseAdmin2.from("users").select("id").eq("role", "worker").eq("is_active", true);
      const userIds = (users || []).map((u) => u.id);
      if (userIds.length > 0) {
        await dispatchBulkNotifications({ userIds, type: "ADMIN_BROADCAST", title, body: msgBody });
        sentCount = userIds.length;
      ...
      const { data: activeBookings } = await supabaseAdmin2.from("bookings").select("customer_id").gte("created_at", thirtyDaysAgo.toISOString());
      const activeIds = (activeBookings || []).map((b) => b.customer_id);
      const { data: customers } = await supabaseAdmin2.from("users").select("id").eq("role", "customer").eq("is_active", true);
      const inactiveIds = (customers || []).map((c) => c.id).filter((id) => !activeIds.includes(id));
      ...
      const { data: users } = await supabaseAdmin2.from("users").select("id").eq("is_active", true);
      const userIds = (users || []).map((u) => u.id);
      if (userIds.length > 0) {
        await dispatchBulkNotifications({ userIds, type: "ADMIN_BROADCAST", title, body: msgBody });
        sentCount = userIds.length;
...
async function GET(request) {
  try {
    ...
    if (phone) {
      const { data, error } = await supabaseAdmin2.from("users").select("id, full_name, phone, role").ilike("phone", `%${phone}%`).limit(5);
    ...
    const [customersRes, workersRes, everyoneRes, activeBookingsRes, recentLogsRes] = await Promise.all([
      supabaseAdmin2.from("users").select("id", { count: "exact", head: true }).eq("role", "customer").eq("is_active", true),
      supabaseAdmin2.from("users").select("id", { count: "exact", head: true }).eq("role", "worker").eq("is_active", true),
      supabaseAdmin2.from("users").select("id", { count: "exact", head: true }).eq("is_active", true),
      supabaseAdmin2.from("bookings").select("customer_id").gte("created_at", thirtyDaysAgo.toISOString()),
      supabaseAdmin2.from("notifications").select("*").eq("type", "ADMIN_BROADCAST_LOG").order("created_at", { ascending: false }).limit(10)
    ]);
    const customerCount = customersRes.count || 0;
    const workerCount = workersRes.count || 0;
    const totalCount = everyoneRes.count || 0;
    const activeCustomerIds = new Set((activeBookingsRes.data || []).map((b) => b.customer_id));
