...
// src/app/api/admin/push-notify/segment/route.ts
var route_exports = {};
...
async function POST(request) {
  try {
    ...
    const { data: { segment, title, body, data }, errorResponse } = await validateBody(request, import_zod2.z.any());
    ...
    if (segment === "ALL_CUSTOMERS") {
      ...
      userIdsToNotify = (users == null ? void 0 : users.map((u) => u.id)) || [];
    } else if (segment === "INACTIVE_30_DAYS") {
      const thirtyDaysAgo = /* @__PURE__ */ new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      const { data: activeUsers } = await supabaseAdmin2.from("bookings").select("customer_id").gte("created_at", thirtyDaysAgo.toISOString());
      const activeUserIds = (activeUsers == null ? void 0 : activeUsers.map((b) => b.customer_id)) || [];
      const { data: users } = await supabaseAdmin2.from("users").select("id").eq("role", "customer").eq("is_active", true);
      userIdsToNotify = ((users == null ? void 0 : users.map((u) => u.id)) || []).filter((id) => !activeUserIds.includes(id));
    } else if (segment === "ALL_WORKERS") {
      const { data: workers } = await supabaseAdmin2.from("workers").select("id");
      userIdsToNotify = (workers == null ? void 0 : workers.map((w) => w.id)) || [];
    } else {
