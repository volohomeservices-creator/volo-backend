...
async function GET(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin2.from("referral_settings").select("*").order("role");
...
async function PUT(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin2.from("referral_settings").update({
