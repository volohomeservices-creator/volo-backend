...
async function GET(request) {
  try {
    ...
    const { data: recentEvents } = await supabaseAdmin.from("security_events").select("*").order("created_at", { ascending: false }).limit(30);
    const { count: activeSessionsCount } = await supabaseAdmin.from("sessions").select("*", { count: "exact", head: true }).eq("is_active", true);
