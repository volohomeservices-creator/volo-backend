...
// src/app/api/admin/workers/[id]/location-history/route.ts
var route_exports = {};
...
async function GET(request, { params }) {
  try {
    ...
    const { data: booking, error: bookingError } = await supabaseAdmin.from("bookings").select("created_at, completed_at, lat, lng").eq("id", bookingId).single();
    if (bookingError || !booking) {
      return NextResponse.json({ error: "Booking not found" }, { status: 404 });
    }
    const startTime = booking.created_at;
    const endTime = booking.completed_at || (/* @__PURE__ */ new Date()).toISOString();
    const { data: locations, error: locError } = await supabaseAdmin.from("worker_location_logs").select("lat, lng, recorded_at").eq("worker_id", workerId).gte("recorded_at", startTime).lte("recorded_at", endTime).order("recorded_at", { ascending: true });
