...
// src/app/api/admin/workers/[id]/strikes/route.ts
var route_exports = {};
...
async function GET(request, { params }) {
  try {
    ...
    const { data, error } = await supabaseAdmin2.from("worker_strikes").select("id, reason, created_at, admin:admin_id(full_name)").eq("worker_id", workerId).order("created_at", { ascending: false });
...
async function POST(request, { params }) {
  try {
    ...
    const { id: workerId } = await params;
    const { data: { reason }, errorResponse } = await validateBody(request, import_zod.z.any());
    if (errorResponse) return errorResponse;
    if (!reason) {
      return NextResponse.json({ error: "Reason is required" }, { status: 400 });
    }
    const { error } = await supabaseAdmin2.from("worker_strikes").insert({
      worker_id: workerId,
