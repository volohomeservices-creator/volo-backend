...
// src/app/api/device-tokens/remove/route.ts
var route_exports = {};
...
async function POST(request) {
  try {
    ...
    const { error: updateErr } = await supabaseAdmin.from("user_devices").update({ is_active: false }).eq("user_id", userId).eq("device_token", deviceToken);
    if (updateErr) {
      throw updateErr;
    }
    await logAuditAction({
      admin_id: userId,
      action: "DEVICE_REMOVED" /* DEVICE_REMOVED */,
      target_type: "user",
      target_id: userId,
