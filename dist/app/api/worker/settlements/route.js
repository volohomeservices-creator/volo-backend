...
// src/app/api/worker/settlements/route.ts
var route_exports = {};
...
// packages/shared-lib/src/lib/settlement-engine.ts
async function calculateWorkerSettlement(workerId) {
  const { data: ledger, error } = await supabaseAdmin.from("settlement_ledger").select("gross_amount, commission_amount, net_amount, status").eq("worker_id", workerId);
...
async function GET(request) {
  ...
  try {
    ...
    const { data: history } = await supabaseAdmin.from("settlement_ledger").select("*, settlement_batches(batch_reference, status)").eq("worker_id", session.user_id).order("created_at", { ascending: false }).limit(20);
