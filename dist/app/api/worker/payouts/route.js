...
// src/app/api/worker/payouts/route.ts
var route_exports = {};
...
// packages/shared-lib/src/lib/payouts/payout-reporting.ts
var import_server_only2 = require("server-only");
async function getWorkerPayoutReport(workerId) {
  const { data, error } = await supabaseAdmin.from("payouts").select("amount, status").eq("worker_id", workerId);
...
async function GET(request) {
  try {
    ...
    const { data: payouts, error } = await supabaseAdmin.from("payouts").select("id, amount, status, created_at, settlement_batches(batch_reference)").eq("worker_id", session.user_id).order("created_at", { ascending: false }).limit(50);
