...
// src/app/api/worker/earnings/route.ts
var route_exports = {};
...
async function GET(request) {
  ...
  try {
    ...
    const { data: bookings, error } = await supabaseAdmin.from("bookings").select("id, total_amount, completed_at").eq("worker_id", workerId).eq("status", "COMPLETED");
    if (error) throw error;
    const now = /* @__PURE__ */ new Date();
    let startDate = (0, import_date_fns.startOfWeek)(now, { weekStartsOn: 1 });
    if (period === "today") {
      startDate = (0, import_date_fns.startOfDay)(now);
    } else if (period === "month") {
      startDate = (0, import_date_fns.startOfMonth)(now);
    } else if (period === "custom" && dateFrom) {
      startDate = (0, import_date_fns.parseISO)(dateFrom);
    }
    const endDate = period === "custom" && dateTo ? (0, import_date_fns.parseISO)(dateTo) : now;
    const filteredBookings = (bookings || []).filter((b) => {
      if (!b.completed_at) return false;
      const completedDate = (0, import_date_fns.parseISO)(b.completed_at);
      return (0, import_date_fns.isWithinInterval)(completedDate, { start: startDate, end: endDate });
    });
    let totalGross = 0;
    ...
    if (period === "today") {
      filteredBookings.forEach((b) => {
        const hour = (0, import_date_fns.format)((0, import_date_fns.parseISO)(b.completed_at), "hh a");
      ...
      filteredBookings.forEach((b) => {
        const dayName = (0, import_date_fns.format)((0, import_date_fns.parseISO)(b.completed_at), "eee");
        chartMap[dayName] = (chartMap[dayName] || 0) + Number((b.total_amount * 0.85).toFixed(2));
      });
    } else {
      filteredBookings.forEach((b) => {
        const dayStr = (0, import_date_fns.format)((0, import_date_fns.parseISO)(b.completed_at), "MMM dd");
    ...
    let { data: wallet } = await supabaseAdmin.from("worker_wallets").select("*").eq("worker_id", workerId).maybeSingle();
    if (!wallet) {
      const { data: newWallet } = await supabaseAdmin.from("worker_wallets").insert({ worker_id: workerId, balance: 0, minimum_balance: -500 }).select("*").single();
      wallet = newWallet;
    }
    const { data: txns } = await supabaseAdmin.from("wallet_transactions").select("*").eq("worker_id", workerId).order("created_at", { ascending: false }).limit(20);
    return NextResponse.json({
      summary: {
        total: totalEarnings,
...
async function POST(request) {
  try {
    ...
    const { error: updateErr } = await supabaseAdmin.from("worker_wallets").update({ balance: newBalance, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("worker_id", workerId);
