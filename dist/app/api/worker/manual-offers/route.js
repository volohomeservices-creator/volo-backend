...
// src/app/api/worker/manual-offers/route.ts
var route_exports = {};
...
async function GET(request) {
  ...
  try {
    ...
    const [workerRes, liveLocRes] = await Promise.all([
      supabaseAdmin.from("workers").select("current_lat, current_lng").eq("id", workerId).single(),
    ...
    const current_lat = liveLoc ? Number(liveLoc.latitude) : worker == null ? void 0 : worker.current_lat;
    const current_lng = liveLoc ? Number(liveLoc.longitude) : worker == null ? void 0 : worker.current_lng;
    const nowIso = (/* @__PURE__ */ new Date()).toISOString();
    const { data: offers, error: fetchErr } = await supabaseAdmin.from("manual_assignment_history").select(`
        id,
        status,
        notes,
        expires_at,
      ...
      `).eq("worker_id", workerId).in("status", ["ASSIGNED", "EXPIRED"]).in("booking.status", ["MANUAL_ASSIGNMENT_REQUIRED", "PENDING_ASSIGNMENT"]);
    if (fetchErr) {
      throw fetchErr;
    }
    const activeOffers = (offers || []).map((o) => {
      ...
      let distance = 0;
      const b = o.booking;
      if (current_lat !== null && current_lng !== null && (b == null ? void 0 : b.lat) !== null && (b == null ? void 0 : b.lng) !== null) {
      ...
      return {
        ...
        bookingId: b.id,
        serviceName: ((_a2 = b.service_items) == null ? void 0 : _a2.name) || "Home Service",
