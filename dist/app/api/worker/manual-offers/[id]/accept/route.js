...
// src/app/api/worker/manual-offers/[id]/accept/route.ts
var route_exports = {};
...
// packages/shared-lib/src/lib/manual-assignment.ts
async function acceptManualAssignmentOffer(offerId, workerId) {
  var _a2;
  const { data: offer, error: offerErr } = await supabaseAdmin.from("manual_assignment_history").select("*").eq("id", offerId).single();
  if (offerErr || !offer) {
  ...
  if (offer.expires_at && new Date(offer.expires_at) < /* @__PURE__ */ new Date()) {
    await supabaseAdmin.from("manual_assignment_history").update({ status: "EXPIRED", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", offerId);
  ...
  const { data: booking, error: bookingErr } = await supabaseAdmin.from("bookings").select("status, customer_id, worker_id").eq("id", bookingId).single();
  ...
  if (acceptErr || !accepted) {
    console.error("[Manual Assignment] auto_accept_booking RPC failed:", acceptErr == null ? void 0 : acceptErr.message);
    return false;
  }
  await supabaseAdmin.from("manual_assignment_history").update({ status: "ACCEPTED", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", offerId);
  ...
  await logAuditAction({
    ...
    action: "MANUAL_ASSIGNMENT_ACCEPTED" /* MANUAL_ASSIGNMENT_ACCEPTED */,
    target_type: "booking",
    target_id: bookingId,
