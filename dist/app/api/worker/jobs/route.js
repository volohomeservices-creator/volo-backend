...
// src/app/api/worker/jobs/route.ts
var route_exports = {};
...
async function GET(request) {
  try {
    ...
    const [workerRes, liveLocRes] = await Promise.all([
      supabaseAdmin.from("workers").select("current_lat, current_lng, status").eq("id", workerId).single(),
    ...
    const current_lat = liveLoc ? Number(liveLoc.latitude) : workerRes.data.current_lat;
    const current_lng = liveLoc ? Number(liveLoc.longitude) : workerRes.data.current_lng;
    if (tab === "new") {
      ...
      const { data: rejections } = await supabaseAdmin.from("worker_job_rejections").select("booking_id").eq("worker_id", workerId);
      const rejectedBookingIds = new Set((rejections || []).map((r) => r.booking_id));
      const { data: manuallyAssignedBookings, error: manualErr } = await supabaseAdmin.from("bookings").select("*, service_items(name), customer:users!bookings_customer_id_fkey(full_name)").eq("status", "WORKER_ASSIGNED").eq("worker_id", workerId);
      if (manualErr) throw manualErr;
      const { data: broadcastingBookings, error: broadcastErr } = await supabaseAdmin.from("bookings").select(`
          *,
          service_items(name),
      ...
      const eligibleBroadcastBookings = (broadcastingBookings || []).filter((b) => {
        var _a2;
        if (rejectedBookingIds.has(b.id)) return false;
        const groupWorkers = ((_a2 = b.assignment_queue) == null ? void 0 : _a2.group_workers) || [];
      ...
      const jobs = mergedBookings.map((b) => {
        var _a2, _b;
        let distance = 0;
        ...
        return {
          id: b.id,
          service_name: ((_a2 = b.service_items) == null ? void 0 : _a2.name) || "Home Service",
          customer_first_name: ((_b = b.customer) == null ? void 0 : _b.full_name) ? b.customer.full_name.split(" ")[0] : "Client",
      ...
      const { data: bookings, error: bookingsErr } = await supabaseAdmin.from("bookings").select("*, service_items(name), customer:users!bookings_customer_id_fkey(full_name)").eq("worker_id", workerId).in("status", ["WORKER_ACCEPTED", "ON_THE_WAY", "ARRIVED", "IN_PROGRESS"]);
      ...
      const { data: bookings, error: bookingsErr } = await supabaseAdmin.from("bookings").select("*, service_items(name), customer:users!bookings_customer_id_fkey(full_name)").eq("worker_id", workerId).in("status", ["COMPLETED", "CANCELLED"]).order("created_at", { ascending: false }).limit(20);
      if (bookingsErr) throw bookingsErr;
      const jobs = (bookings || []).map((b) => {
        var _a2, _b;
        return {
