...
// src/app/api/worker/referrals/route.ts
var route_exports = {};
...
async function GET(request) {
  try {
    ...
    let { data: codeRow } = await supabaseAdmin.from("referral_codes").select("*").eq("user_id", userId).eq("role", "worker").single();
    if (!codeRow) {
      const { data: user } = await supabaseAdmin.from("users").select("full_name").eq("id", userId).single();
      const code = generateCode((user == null ? void 0 : user.full_name) || "WRK");
      const { data: newCode } = await supabaseAdmin.from("referral_codes").insert({ user_id: userId, referral_code: code, role: "worker" }).select().single();
      codeRow = newCode;
    }
    const { data: referrals } = await supabaseAdmin.from("referrals").select("id, status, reward_amount, created_at").eq("referrer_id", userId).eq("role", "worker").order("created_at", { ascending: false });
    ...
    const { data: settings } = await supabaseAdmin.from("referral_settings").select("referrer_reward, referee_reward, min_bookings_to_qualify").eq("role", "worker").eq("active", true).single();
