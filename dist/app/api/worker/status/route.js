...
async function PATCH(request) {
  try {
    ...
    const { data: worker, error: workerErr } = await supabaseAdmin.from("workers").select("kyc_status, status").eq("id", workerId).single();
    if (workerErr || !worker) {
    ...
    let lat = latitude !== void 0 && latitude !== null ? Number(latitude) : null;
    let lng = longitude !== void 0 && longitude !== null ? Number(longitude) : null;
    if (status === "ONLINE" && (lat === null || lng === null)) {
      const { data: currentWorker } = await supabaseAdmin.from("workers").select("current_lat, current_lng").eq("id", workerId).single();
      if (currentWorker && currentWorker.current_lat !== null && currentWorker.current_lng !== null) {
        ...
        const { data: profile } = await supabaseAdmin.from("worker_profiles").select("address, city, state").eq("worker_id", workerId).maybeSingle();
        if (profile) {
    ...
    const { error: updateErr } = await supabaseAdmin.from("workers").update(updatePayload).eq("id", workerId);
    if (updateErr) throw updateErr;
    if (status === "VACATION") {
      await supabaseAdmin.from("worker_availability").update({ vacation_mode: true }).eq("worker_id", workerId);
    } else {
      await supabaseAdmin.from("worker_availability").update({ vacation_mode: false }).eq("worker_id", workerId);
    }
    return NextResponse.json({
      success: true,
      status,
