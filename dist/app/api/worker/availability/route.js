...
async function GET(request) {
  try {
    ...
    let { data: availability, error } = await supabaseAdmin.from("worker_availability").select("*").eq("worker_id", workerId).maybeSingle();
    if (error) throw error;
    if (!availability) {
      const { data: newAvail } = await supabaseAdmin.from("worker_availability").insert({ worker_id: workerId }).select().single();
...
async function PUT(request) {
  try {
    ...
    const { error: updateAvailErr } = await supabaseAdmin.from("worker_availability").upsert({
      worker_id: workerId,
    ...
    if (vacation_mode) {
      await supabaseAdmin.from("workers").update({ status: "VACATION" }).eq("id", workerId);
    } else {
      const { data: worker } = await supabaseAdmin.from("workers").select("status").eq("id", workerId).single();
      if ((worker == null ? void 0 : worker.status) === "VACATION") {
