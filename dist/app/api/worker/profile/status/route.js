...
// src/app/api/worker/profile/status/route.ts
var route_exports = {};
...
async function PATCH(request) {
  try {
    ...
    const { data: { status }, errorResponse } = await validateBody(request, import_zod.z.any());
    if (errorResponse) return errorResponse;
    if (status !== "ONLINE" && status !== "OFFLINE") {
      return NextResponse.json({ error: "Invalid status." }, { status: 400 });
    }
    const { error: updateErr } = await supabaseAdmin.from("workers").update({
      status,
