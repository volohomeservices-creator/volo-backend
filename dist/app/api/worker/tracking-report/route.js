...
// src/app/api/worker/tracking-report/route.ts
var route_exports = {};
...
async function getWorkerTrackingReport(workerId) {
  try {
    const { data: snaps } = await supabaseAdmin.from("booking_route_snapshots").select("distance_km, bookings(worker_id)").eq("bookings.worker_id", workerId);
    let distanceCoveredKm = 0;
    if (snaps) {
      distanceCoveredKm = snaps.filter((s) => s.bookings && s.bookings.worker_id === workerId).reduce((acc, curr) => acc + Number(curr.distance_km || 0), 0);
    }
    const { data: sessions } = await supabaseAdmin.from("tracking_sessions").select("started_at, ended_at, status").eq("worker_id", workerId);
