...
// src/app/api/customer/services/[id]/route.ts
var route_exports = {};
...
async function GET(request, { params }) {
  try {
    ...
    let item = null;
    if (isUuid) {
      const { data, error } = await supabaseAdmin.from("service_items").select("*, service_categories(id, name, icon_url)").eq("id", idOrSlug).single();
      if (!error && data) {
        item = data;
      }
    }
    if (!item) {
      const { data: allItems, error: fetchErr } = await supabaseAdmin.from("service_items").select("*, service_categories(id, name, icon_url)").eq("is_active", true);
