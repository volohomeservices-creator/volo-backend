...
// src/app/api/customer/banners/route.ts
var route_exports = {};
...
async function GET() {
  try {
    const { data: banners, error } = await supabaseAdmin2.from("mobile_banners").select("*").eq("active", true).order("created_at", { ascending: false });
    if (error) throw error;
    const { data: promos } = await supabaseAdmin2.from("promo_codes").select("*").eq("active", true).order("created_at", { ascending: false });
