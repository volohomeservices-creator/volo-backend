...
// src/app/api/customer/prime/route.ts
var route_exports = {};
...
async function POST(request) {
  try {
    ...
    const { data: { action }, errorResponse } = await validateBody(request, import_zod.z.any());
    if (errorResponse) return errorResponse;
    if (action !== "SUBSCRIBE" && action !== "UNSUBSCRIBE") {
      return NextResponse.json({ error: "Invalid action" }, { status: 400 });
    }
    const isPrime = action === "SUBSCRIBE";
    const { error } = await supabaseAdmin.from("users").update({ is_prime: isPrime }).eq("id", session.user_id);
