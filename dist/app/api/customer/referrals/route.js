...
// src/app/api/customer/referrals/route.ts
var route_exports = {};
...
// src/app/api/customer/referrals/route.ts
function generateCode(name, role) {
...
async function GET(request) {
  try {
    ...
    let { data: codeRow } = await supabaseAdmin.from("referral_codes").select("*").eq("user_id", userId).eq("role", "customer").single();
    if (!codeRow) {
      const { data: user } = await supabaseAdmin.from("users").select("full_name").eq("id", userId).single();
      const code = generateCode((user == null ? void 0 : user.full_name) || "USER", "customer");
      const { data: newCode } = await supabaseAdmin.from("referral_codes").insert({ user_id: userId, referral_code: code, role: "customer" }).select().single();
      codeRow = newCode;
    }
    const { data: referrals } = await supabaseAdmin.from("referrals").select("id, status, reward_amount, created_at, referred_user_id").eq("referrer_id", userId).eq("role", "customer").order("created_at", { ascending: false });
    ...
    const totalEarned = (referrals || []).filter((r) => r.status === "REWARDED").reduce((sum, r) => sum + Number(r.reward_amount || 0), 0);
    const pendingAmount = (referrals || []).filter((r) => r.status === "QUALIFIED").reduce((sum, r) => sum + Number(r.reward_amount || 0), 0);
    const { data: settings } = await supabaseAdmin.from("referral_settings").select("referrer_reward, referee_reward, min_bookings_to_qualify").eq("role", "customer").eq("active", true).single();
    return NextResponse.json({
      code: (codeRow == null ? void 0 : codeRow.referral_code) || null,
      referrals: referrals || [],
      stats: {
        successCount,
        pendingCount,
