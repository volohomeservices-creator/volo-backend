...
async function GET(request) {
  try {
    ...
    const { data: user, error: userErr } = await supabaseAdmin.from("users").select("id, full_name, email, avatar_url, is_prime, volo_coins").eq("id", customerId).single();
    if (userErr || !user) {
      return NextResponse.json({ error: "User not found" }, { status: 404 });
    }
    const { data: bookings, error: bookingsErr } = await supabaseAdmin.from("bookings").select("id, customer_id, worker_id, service_item_id, booking_type, payment_mode, status, address_line, lat, lng, scheduled_at, started_at, completed_at, total_amount, notes, otp, created_at, updated_at, service_items(name), workers(users(full_name), worker_live_locations_approx(latitude, longitude))").eq("customer_id", customerId).order("created_at", { ascending: false });
    if (bookingsErr) throw bookingsErr;
    const { data: reviews, error: reviewsErr } = await supabaseAdmin.from("reviews").select("id, booking_id, customer_id, worker_id, rating, comment:review_text, created_at, bookings(service_items(name))").eq("customer_id", customerId).order("created_at", { ascending: false }).limit(5);
    if (reviewsErr) throw reviewsErr;
    const totalBookings = bookings.length;
    const completedBookings = bookings.filter((b) => b.status === "COMPLETED").length;
    const pendingBookings = bookings.filter(
      (b) => b.status === "PENDING_ASSIGNMENT" || b.status === "WORKER_ASSIGNED" || b.status === "WORKER_ACCEPTED"
    ).length;
    const nowStr = (/* @__PURE__ */ new Date()).toISOString();
    const upcomingBookings = bookings.filter(
      (b) => b.status !== "COMPLETED" && b.status !== "CANCELLED" && b.scheduled_at && b.scheduled_at > nowStr
    ).length;
    const activeList = bookings.filter(
      (b) => b.status === "ON_THE_WAY" || b.status === "ARRIVED" || b.status === "IN_PROGRESS"
    );
    ...
    if (defaultAddr) {
      if (defaultAddr.address) completedFields += 3;
    }
    const profileCompletion = Math.min(100, Math.round(completedFields / totalFields * 100));
    const { data: promos } = await supabaseAdmin.from("promo_codes").select("*").eq("active", true).order("created_at", { ascending: false });
    ...
    return NextResponse.json({
      ...
      upcomingBookingsList: bookings.filter(
        (b) => b.status !== "COMPLETED" && b.status !== "CANCELLED" && b.scheduled_at && b.scheduled_at > nowStr
      ).slice(0, 5),
