...
async function GET(request) {
  try {
    ...
    const { data: user, error: userErr } = await supabaseAdmin.from("users").select("volo_coins").eq("id", session.user_id).single();
    if (userErr) throw userErr;
    const { data: history, error: historyErr } = await supabaseAdmin.from("volo_coin_transactions").select("*").eq("user_id", session.user_id).order("created_at", { ascending: false });
    return NextResponse.json({
      coins: (user == null ? void 0 : user.volo_coins) || 0,
      history: history || []
    });
  } catch (error) {
...
async function POST(request) {
  try {
    const session = await requireRole(request, "customer");
    const { data: { amountToConvert }, errorResponse } = await validateBody(request, import_zod.z.any());
    if (errorResponse) return errorResponse;
    if (!amountToConvert || amountToConvert <= 0) {
      return NextResponse.json({ error: "Invalid amount" }, { status: 400 });
    }
    const { data: user } = await supabaseAdmin.from("users").select("volo_coins").eq("id", session.user_id).single();
    ...
    await supabaseAdmin.from("users").update({ volo_coins: user.volo_coins - amountToConvert }).eq("id", session.user_id);
    await supabaseAdmin.from("volo_coin_transactions").insert({
      user_id: session.user_id,
      amount: -amountToConvert,
      description: `Converted to \u20B9${walletCredit} wallet balance`
    });
    const { data: wallet } = await supabaseAdmin.from("customer_wallets").select("*").eq("customer_id", session.user_id).single();
    if (wallet) {
      await supabaseAdmin.from("customer_wallets").update({ balance: Number(wallet.balance) + walletCredit }).eq("id", wallet.id);
