...
// src/app/api/customer/booking-history/route.ts
var route_exports = {};
...
async function GET(request) {
  try {
    ...
    const { data: bookings, count, error } = await supabaseAdmin.from("bookings").select("*, service_items(name, description, base_price, estimated_mins), workers(users(full_name)), reviews(rating, comment:review_text)", { count: "exact" }).eq("customer_id", customerId).in("status", ["COMPLETED", "CANCELLED"]).order("created_at", { ascending: false }).range(offset, offset + limit - 1);
