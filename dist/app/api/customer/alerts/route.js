...
// src/app/api/customer/alerts/route.ts
var route_exports = {};
...
async function GET(request) {
  try {
    ...
    const { data: notifications, error } = await supabaseAdmin.from("notifications").select("*").eq("user_id", customerId).order("created_at", { ascending: false }).limit(50);
...
async function DELETE(request) {
  try {
    const session = await requireRole(request, "customer");
    const customerId = session.user_id;
    const { error } = await supabaseAdmin.from("notifications").delete().eq("user_id", customerId);
    if (error) throw error;
