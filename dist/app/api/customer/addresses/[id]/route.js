...
async function PATCH(request, { params }) {
  try {
    ...
    const { data: existingAddr } = await supabaseAdmin.from("customer_addresses").select("*").eq("id", id).single();
    if (!existingAddr || existingAddr.customer_id !== customerId) {
      return NextResponse.json({ error: "Access denied." }, { status: 403 });
    }
    let lat = latitude !== void 0 ? Number(latitude) : existingAddr.latitude;
    let lng = longitude !== void 0 ? Number(longitude) : existingAddr.longitude;
    if (address !== void 0 && address !== existingAddr.address || lat === 12.9716 && lng === 77.5946) {
    ...
    const { data: updatedAddr, error } = await supabaseAdmin.from("customer_addresses").update({
      label,
      address,
      latitude: lat,
      longitude: lng,
      is_default: is_default !== void 0 ? !!is_default : existingAddr.is_default,
      place_id: place_id !== void 0 ? place_id : existingAddr.place_id,
      formatted_address: formatted_address !== void 0 ? formatted_address : existingAddr.formatted_address,
