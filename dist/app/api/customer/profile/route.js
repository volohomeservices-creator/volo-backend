...
// src/app/api/customer/profile/route.ts
function getCompletionPercentage(user, defaultAddr) {
  ...
  if (defaultAddr == null ? void 0 : defaultAddr.address) completedFields += 3;
...
async function GET(request) {
  try {
    ...
    const { data: user, error: userErr } = await supabaseAdmin.from("users").select("full_name, phone, email, avatar_url").eq("id", customerId).single();
    if (userErr || !user) {
    ...
    return NextResponse.json({
      ...
      address: (defaultAddr == null ? void 0 : defaultAddr.address) || "",
      city: (defaultAddr == null ? void 0 : defaultAddr.city) || (defaultAddr ? "Bangalore" : ""),
      state: (defaultAddr == null ? void 0 : defaultAddr.state) || (defaultAddr ? "Karnataka" : ""),
      pincode: (defaultAddr == null ? void 0 : defaultAddr.pincode) || "",
...
async function PATCH(request) {
  ...
  try {
    const session = await requireRole(request, "customer");
    const customerId = session.user_id;
    const validation = await validateBody(request, customerProfileUpdateSchema);
    ...
    if (address !== void 0 || city !== void 0 || state !== void 0 || pincode !== void 0) {
      const { data: existingAddr } = await supabaseAdmin.from("customer_addresses").select("*").eq("customer_id", customerId).eq("is_default", true).maybeSingle();
      if (existingAddr) {
        const targetAddress = address !== void 0 ? address : existingAddr.address;
        const targetCity = city !== void 0 ? city : existingAddr.city;
        const targetState = state !== void 0 ? state : existingAddr.state;
        const targetPincode = pincode !== void 0 ? pincode : existingAddr.pincode;
        ...
        await supabaseAdmin.from("customer_addresses").update({
...
async function DELETE(request) {
  try {
    const session = await requireRole(request, "customer");
    const customerId = session.user_id;
    const { error } = await supabaseAdmin.from("users").update({ is_active: false }).eq("id", customerId);
