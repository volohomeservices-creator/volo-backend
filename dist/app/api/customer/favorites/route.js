...
async function GET(request) {
  try {
    ...
    const { data: favorites, error } = await supabaseAdmin.from("customer_favorites").select(`
        id,
        created_at,
        worker_id,
        workers (
      ...
      `).eq("customer_id", session.user_id).order("created_at", { ascending: false });
    if (error) {
...
async function POST(request) {
  try {
    const session = await requireRole(request, "customer");
    const { data: { worker_id, action }, errorResponse } = await validateBody(request, import_zod.z.any());
    if (errorResponse) return errorResponse;
    if (!worker_id || !action) {
      return NextResponse.json({ error: "Worker ID and action required" }, { status: 400 });
    }
    if (action === "ADD") {
      const { data, error } = await supabaseAdmin.from("customer_favorites").insert({
        customer_id: session.user_id,
        worker_id
      });
      if (error && error.code !== "23505") throw error;
    } else if (action === "REMOVE") {
      const { error } = await supabaseAdmin.from("customer_favorites").delete().eq("customer_id", session.user_id).eq("worker_id", worker_id);
