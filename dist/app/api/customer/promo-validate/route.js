...
// src/app/api/customer/promo-validate/route.ts
var route_exports = {};
...
async function POST(request) {
  try {
    ...
    const { data: { code, order_amount }, errorResponse } = await validateBody(request, import_zod.z.any());
    if (errorResponse) return errorResponse;
    if (!code) {
      return NextResponse.json({ error: "Promo code is required" }, { status: 400 });
    }
    const { data: promo, error: fetchErr } = await supabaseAdmin.from("promo_codes").select("*").eq("code", code.trim().toUpperCase()).eq("active", true).single();
    if (fetchErr || !promo) {
    ...
    const { data: existingUsage } = await supabaseAdmin.from("promo_code_usages").select("id").eq("promo_code_id", promo.id).eq("user_id", session.user_id).single();
    if (existingUsage) {
