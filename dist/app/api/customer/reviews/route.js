...
async function GET(request) {
  try {
    ...
    const { data: reviews, error } = await supabaseAdmin.from("reviews").select("*, bookings(service_items(name)), workers(users(full_name))").eq("customer_id", customerId).order("created_at", { ascending: false });
    if (error) throw error;
    const formattedReviews = (reviews || []).map((r) => __spreadProps(__spreadValues({}, r), {
...
async function POST(request) {
  try {
    ...
    const { data: newReview, error: reviewErr } = await supabaseAdmin.from("reviews").insert({
      booking_id,
    ...
    const { data: allReviews, error: aggErr } = await supabaseAdmin.from("reviews").select("rating").eq("worker_id", workerId);
    if (!aggErr && allReviews && allReviews.length > 0) {
      const avgRating = allReviews.reduce((sum, r) => sum + Number(r.rating), 0) / allReviews.length;
      await supabaseAdmin.from("workers").update({ rating: parseFloat(avgRating.toFixed(2)) }).eq("id", workerId);
    }
    return NextResponse.json({
      success: true,
      review: newReview,
