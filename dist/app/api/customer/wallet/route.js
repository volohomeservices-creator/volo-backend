...
async function GET(request) {
  ...
  try {
    ...
    let { data: wallet, error: walletErr } = await supabaseAdmin.from("customer_wallets").select("*").eq("customer_id", customerId).maybeSingle();
    if (walletErr) throw walletErr;
    if (!wallet) {
      const { data: newWallet, error: createErr } = await supabaseAdmin.from("customer_wallets").insert({
    ...
    const { data: transactions, error: txErr } = await supabaseAdmin.from("customer_wallet_transactions").select("*").eq("customer_id", customerId).order("created_at", { ascending: false }).limit(20);
...
async function POST(request) {
  try {
    ...
    let { data: wallet } = await supabaseAdmin.from("customer_wallets").select("*").eq("customer_id", customerId).maybeSingle();
    if (!wallet) {
      const { data: newWallet } = await supabaseAdmin.from("customer_wallets").insert({ customer_id: customerId, balance: 0 }).select("*").single();
      wallet = newWallet;
    }
    if (!wallet) {
    ...
    const { error: updateErr } = await supabaseAdmin.from("customer_wallets").update({ balance: newBalance, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("customer_id", customerId);
