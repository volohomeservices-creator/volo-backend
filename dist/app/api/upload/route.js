...
async function POST(request) {
  try {
    ...
    const bucket = formData.get("bucket") || "booking-images";
    ...
    const { data: uploadData, error: uploadErr } = await supabaseAdmin.storage.from(bucket).upload(path, buffer, {
    ...
    return NextResponse.json({ error: (error == null ? void 0 : error.message) || "Upload failed" }, { status: 500 });
  }
}
