...
async function POST(request) {
  ...
  try {
    ...
    if (session) {
      const accessToken = (_a2 = request.cookies.get("volo_session")) == null ? void 0 : _a2.value;
      if (accessToken) {
        const tokenHash = import_crypto2.default.createHash("sha256").update(accessToken).digest("hex");
        await supabaseAdmin.from("sessions").update({ is_active: false }).eq("access_token_hash", tokenHash);
      }
    }
